...
function userPost(userPost, firstName, lastName) {
  userPost.forEach((post) => {
    ...
    let comments = post.comments.reduce((allComments, comments) => {
      ...
      return (allComments += `<p>${comments.username} : ${comments.title} <span class="time">${comments.time}</span></p>`);
    ...
    $(`.comment${post.id}`).animate(
      { scrollTop: $(`.comment${post.id}`).height() * 100000 },
      1000
    );
  });
}
    ...
    axios
      ...
      .then(({ data }) => {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          notfriendElem.innerHTML = `<div class="friendbox"><button> ${data[i].firstName} ${data[i].lastName}</button><button id='add' onclick=addFriend('${data[i].uuid}')><span>➕</span></button></div>`;
...
function generateComment(commId, user, value, time) {
  $(`${commId}`).append(
    `<p>${user}: ${value} <span class="time">${time}</span></p>`
  );
}
          ...
          axios
            ...
            .then((message) => {
              socket.emit("Update", [
                ...
                `<p>${message.data.username} : ${message.data.title} <span class="time">${message.data.time}</span></p>`,
              ]);
              event.target.text.value = "";
              $(`.comment${event.target.dataset.postid}`).animate(
                {
                  scrollTop:
                    $(`.comment${event.target.dataset.postid}`).height() *
                    100000,
                },
                1000
              );
