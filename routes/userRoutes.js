...
router.get("/users/:username/:password/login", (req, res) => {
  User.findOne({
    ...
    .then((data) =>
      data === null ? res.json("Not Found") : res.json(data.dataValues.uuid)
    )
...
router.get("/users/info/:userid", (req, res) => {
  User.findAll({
    ...
    .then(async (data) => {
      ...
      let friends = data[0].friend.map((val) => {
        return {
...
router.put("/users/update/:password/:uuid", (req, res) => {
  User.update(req.body, {
    where: { password: req.params.password, id: req.params.uuid },
  })
...
router.delete("/users/delete/:password/:uuid", (req, res) => {
  User.destroy({
    where: { password: req.params.password, uuid: req.params.uuid },
  })
    .then(() => res.sendStatus(200))
    .catch((err) => console.error(err));
});
